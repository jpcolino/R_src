...
prices10 <- read.csv("coalpower10.csv", header = TRUE, sep = ",", quote="\"", dec=".")
prices10 <- prices10[complete.cases(prices10),]
prices10 <- prices10[1:dim(prices10)[1],]

prices11 <- read.csv("coalpower11.csv", header = TRUE, sep = ",", quote="\"", dec=".")
prices11 <- prices11[complete.cases(prices11),]
prices11 <- prices11[1:dim(prices11)[1],]

prices12 <- read.csv("coalpower12.csv", header = TRUE, sep = ",", quote="\"", dec=".")
prices12 <- prices12[complete.cases(prices12),]
prices12 <- prices12[1:dim(prices12)[1],]

prices13 <- read.csv("coalpower13.csv", header = TRUE, sep = ",", quote="\"", dec=".")
prices13 <- prices13[complete.cases(prices13),]
prices13 <- prices13[1:dim(prices13)[1],]

prices14 <- read.csv("coalpower14.csv", header = TRUE, sep = ",", quote="\"", dec=".")
prices14 <- prices14[complete.cases(prices14),]
prices14 <- prices14[1:dim(prices14)[1],]
...
# ================================================================================
# SPREADS FORMULA API2_Euros - Cal10
# ================================================================================
spread10 <- 10*365*24*prices10[,4] - 30000*prices10[,2]/prices10[,3] # Baseload Germany Cal10 - API2_Euros Cal10
spread11 <- 10*365*24*prices11[,4] - 30000*prices11[,2]/prices11[,3] # Baseload Germany Cal11 - API2_Euros Cal11
spread12 <- 10*365*24*prices12[,4] - 30000*prices12[,2]/prices12[,3] # Baseload Germany Cal12 - API2_Euros Cal12
spread13 <- 10*365*24*prices13[,4] - 30000*prices13[,2]/prices13[,3] # Baseload Germany Cal13 - API2_Euros Cal13
spread14 <- 10*365*24*prices14[,4] - 30000*prices14[,2]/prices14[,3] # Baseload Germany Cal13 - API2_Euros Cal13
spread   <- c(spread10,spread11, spread12, spread13, spread14)

# ================================================================================
# PLOTTING the PRICES by YEAR (with ggplot)
# ================================================================================
x10 <- as.Date(prices10$Timestamp,format='%d/%m/%Y')
df_prices10 <- data.frame(x10,prices10$API2Cal10)
names(df_prices10)<- c('x','API2Cal10')
print (ggplot(df_prices10, aes(x10,df_prices10$API2Cal10))+geom_line(colour="white", size=1)+ggtitle("API2Cal10 Prices for 2009"))
# print(ggplot(df_prices10, aes(x10,df_prices10$API2Cal10))+geom_area())


x11 <- as.Date(prices11$Timestamp,format='%d/%m/%Y')
df_prices11 <- data.frame(x11,prices11[,2])
names(df_prices11)<- c('x','API2Cal11')
print(ggplot(df_prices11, aes(x11,df_prices11$API2Cal11))+geom_line(colour="darkgreen", size=1)+ggtitle("API2Cal11 Prices for 2010"))

x12 <- as.Date(prices12$Timestamp,format='%d/%m/%Y')
df_prices12 <- data.frame(x12,prices12[,2])
names(df_prices12)<- c('x','API2Cal12')
print(ggplot(df_prices12, aes(x12,df_prices12$API2Cal12))+geom_line(colour="darkred", size=1)+ggtitle("API2Cal12 Prices for 2011"))

x13 <- as.Date(prices13$Timestamp,format='%d/%m/%Y')
df_prices13 <- data.frame(x13,prices13[,2])
names(df_prices13)<- c('x','API2Cal13')
print(ggplot(df_prices13, aes(x13,df_prices13$API2Cal13))+geom_line(colour="darkblue", size=1)+ggtitle("API2Cal13 Prices for 2012"))

x14 <- as.Date(prices14$Timestamp,format='%d/%m/%Y')
df_prices14 <- data.frame(x14,prices14[,2])
names(df_prices14)<- c('x','API2Cal14')
print(ggplot(df_prices14, aes(x14,df_prices14$API2Cal14))+geom_line(colour="darkblue", size=1)+ggtitle("API2Cal14 Prices for 2012"))
...
m <-min(length(prices10$Timestamp),length(prices11$Timestamp),length(prices12$Timestamp),length(prices13$Timestamp),length(prices14$Timestamp))

df_prices <- data.frame(x14,prices10[1:m,2],prices11[1:m,2],prices12[1:m,2],prices13[1:m,2],prices14[1:m,2])
df_prices <- df_prices[complete.cases(df_prices),]
names(df_prices)<- c('x','w10','w11','w12','w13','w14')

print(ggplot(df_prices, aes(x))+geom_line(aes(y = w14),colour='black',size=1)+
  geom_line(aes(y = w13),colour='red',size=1)+
  geom_line(aes(y = w12),colour='blue',size=1)+
  geom_line(aes(y = w11),colour='green',size=1)+
  ggtitle("API2 Cal Prices for 2011-12-13"))
...
df_spreads <- data.frame(x13,spread10[1:m],spread11[1:m],spread12[1:m],spread13[1:m],spread14[1:m])
df_spreads <- df_spreads[complete.cases(df_spreads),]
names(df_spreads)<- c('x','s10','s11','s12','s13','s14')

D_spreads <- data.frame(x13[2:m],diff(spread10[1:m]),diff(spread11[1:m]),diff(spread12[1:m]),diff(spread13[1:m]),diff(spread14[1:m]))
names(D_spreads)<- c('x','Ds10','Ds11','Ds12','Ds13','Ds14')

sma10<-SMA(df_spreads$s10, n=10)
sma11<-SMA(df_spreads$s11, n=10)
sma12<-SMA(df_spreads$s12, n=10)
sma13<-SMA(df_spreads$s13, n=10)
sma14<-SMA(df_spreads$s14, n=10)

g <- ggplot(df_spreads, aes(x=df_spreads$x))

print(g+
  geom_line(aes(y = sma14),colour='black',size=1.2)+
  geom_line(aes(y = df_spreads$s14),linetype="dashed",colour='darkred',size=.1)+
  ...
  ggtitle("PowerBsldCalGER - API2 Cal Spreads for 2010-to-13"))
...
print(g+
  ...
  geom_area(aes(y = df_spreads$s14), fill="black", alpha=0.1)+
  # scale_fill_brewer(palette="Blues") +
  scale_colour_brewer(palette="Set1")+
  ggtitle("PowerBsldCalGER - API2 Cal Spreads for 2011-to-2014"))
...
print(ggplot(df_spreads, aes(x=df_spreads$s10)) + 
    ...
    geom_histogram(aes(x=df_spreads$s14),fill="black", alpha=0.1) + 
    ggtitle("Histogram Spreads for 2010 - 2011 - 2012 - 2013- 2014"))
...
d1<- ggplot(df_spreads, aes(x=df_spreads$s10)) + 
     ...
     eom_density(aes(x=df_spreads$s14),fill="black", alpha=0.1) +
     ggtitle("Densities of the Spreads for 2010(white) - 2011(green) - 2012(red) - 2013(blue) - 2014(dark)")
...
d2<-ggplot(D_spreads, aes(x=D_spreads$Ds10)) + 
    ...
    eom_density(aes(x=df_spreads$Ds14),fill="black", alpha=0.1) +
    ggtitle("Densities of the Diff of Spreads for 2010(white) - 2011(green) - 2012(red) - 2013(blue)- 2014(dark)")
...
MovingVol11 <- volatility(abs(df_spreads$s11), n=20, calc='close')
MovingVol12 <- volatility(abs(df_spreads$s12), n=20, calc='close')
MovingVol13 <- volatility(abs(df_spreads$s13), n=20, calc='close')
MovingVol14 <- volatility(abs(df_spreads$s14), n=20, calc='close')

m <-min(length(MovingVol10),length(MovingVol11),length(MovingVol12),length(MovingVol13),length(MovingVol14))

df_vols <- data.frame(x14,MovingVol10[1:m],MovingVol11[1:m],MovingVol12[1:m],MovingVol13[1:m],MovingVol14[1:m])
names(df_vols)<- c('x','v10','v11','v12','v13','v14')

g <- ggplot(df_vols, aes(x=x))

print(g+
  geom_line(aes(y = v14),colour='black',size=1.2)+
  ...
  # scale_colour_brewer(palette="Set1")+
  ggtitle("PowerBsldCalGER - API2 Cal Volatilities for 2011-to-14"))
...
d3 <- ggplot(df_vols, aes(x=df_vols$v12))+ 
        ...
        geom_density(aes(x=df_vols$v14),fill="black", alpha=0.1) + 
        ggtitle("Densities of vols over the Diff(prices) for 2010(white) - 2011(green) - 2012(red) - 2013(blue) - 2014(black)")
...
MovingCor13 <-MovingCor(as.numeric(diff(prices13[,4])), as.numeric(diff(prices13[,2])),window.size=21, method="pearson" )
MovingCor14 <-MovingCor(as.numeric(diff(prices14[,4])), as.numeric(diff(prices14[,2])),window.size=21, method="pearson" )
...
m <-min(length(MovingCor10),length(MovingCor11),length(MovingCor12),length(MovingCor13),length(MovingCor14))

df_Cor <- data.frame( x13[1:m], as.numeric(MovingCor10[1:m]), as.numeric(MovingCor11[1:m]),+
                        as.numeric(MovingCor12[1:m]), as.numeric(MovingCor13[1:m]), as.numeric(MovingCor14[1:m]) ) 

names(df_Cor)<- c('x','c10', 'c11','c12','c13','c14')

g <- ggplot(df_Cor, aes(x=df_Cor$x))

print(g+
      geom_line (aes(y = df_Cor$c14), colour='black',  size=1.2)+
      ...
      ggtitle("PowerBsldCalGER - API2 Cal Correlation for 2011-to-14"))
...
d4 <- ggplot(df_Cor, aes(x=df_Cor$c10)) + 
      ...
      geom_density(aes(x=df_Cor$c14),fill="black", alpha=0.1) + 
      ggtitle("Densities of Correlation over the Diff(prices) for 2010(white) - 2011(green) - 2012(red) - 2013(blue) - 2014(black)")
...
# ================================================================================
# Computing the OU-INTERVALS  and plotting Out of sample
# ================================================================================
# 2010
# ================================================================================
...
# ==================================================================
# Recalculation of Means and Variances OUT-OF-SAMPLE
# ==================================================================
...
p21 <-   ggplot(df_prices211, aes(x = df_prices211$x)) + 
         ...
         geom_ribbon (aes(ymin = (df_prices211$mean_10 - 2 * df_prices211$var_10), 
                          ymax = (df_prices211$mean_10 + 2 * df_prices211$var_10)), alpha=0.2) +
         ggtitle('2011 Power-Coal Spread and MeanRev IC from 2010')
...
p22 <-   ggplot(df_prices212, aes(x = df_prices212$x)) + 
         ...
         geom_ribbon (aes(ymin = (df_prices212$mean_11 - 2 * df_prices212$var_11), 
                          ymax = (df_prices212$mean_11 + 2 * df_prices212$var_11)), alpha=0.2) +
         ggtitle('2012 Power-Coal Spread and MeanRev IC from 2011')
...
p23 <-   ggplot(df_prices213, aes(x = df_prices213$x)) + 
         ...
         geom_ribbon (aes(ymin = (df_prices213$mean_12 - 2 * df_prices213$var_12), 
                          ymax = (df_prices213$mean_12 + 2 * df_prices213$var_12)), alpha=0.2) +
         ggtitle('2013 Power-Coal Spread and MeanRev IC from 2012')
