...
spread13 <- 10*182*24*prices13[,2]-10000*prices13[,3]
...
plot.ts(c(prices10[,4], prices11[,4], prices12[,4],  prices13[,3]), main = 'Price evolution Oil Euros 2010 - 2013')
...
print(results12)
  ...
  ts.plot( as.ts(spread131), as.ts(mean_12), as.ts(mean_12 + 1 * var_12), as.ts(mean_12 - 1 * var_12), 
         main='2013 Spread without Local Drift and 2 sigma-IC')
...
plot.ts(cumsum(results11), main='Final Results 2011')
plot.ts(cumsum(results12), main='Final Results 2012')
plot.ts(cumsum(results13), main='Final Results 2013')
